        ...
        Ok(match request {
            ...
            Request::StartAllServices => {
                if self.mode == rinit_service::Mode::Root {
                    // In root mode, start all boot services first
                    graph.start_target(rinit_service::graph::Target::Boot).await;
                }
                // Then start the default target, which is common for all modes
                graph
            ...
            Request::StopAllServices { action } => {
                *self.final_action.borrow_mut() = match action {
                ...
                graph
                    ...
                    .await;
                if self.mode == rinit_service::Mode::Root {
                    // In root mode, stop all boot services last
                    graph
